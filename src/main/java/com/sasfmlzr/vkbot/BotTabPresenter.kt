...
class BotTabPresenter {
    ...
    fun fillLog(): String {
        if (StaticModel.userBot.botApiClient().stateBot.botWork) {
            ...
            statisticMsg = statisticMsg + "Время работы бота равно " + (timeProgramItog / 1000).toFloat().roundToInt() + "c\n" +
                    "Среднее время запроса до вк равно " + (100 * StatisticsVariable.timeZaprosFinishSumm / StatisticsVariable.countSendMessageUser).toFloat().roundToInt() / 100 + "мс\n" +
